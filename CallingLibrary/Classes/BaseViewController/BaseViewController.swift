...
open class BaseViewController:UIViewController{
   ...
   // var container_view_height:CGFloat = 200
    ...
    open override func viewWillAppear(_ animated: Bool) {
        ...
        getIPDetails()
        country_model.baseView = self
        country_model.getAllCountriesApiCalling()
        print("List of Country",country_model.countriesArray)
...
//MARK: - EMAIL SetupUI
    ...
    public func getIPDetails() {
        guard let url = URL(string: WebService.method.getipaddressData.rawValue) else {
           // completion(nil)
            return
        }
        var request = URLRequest(url: url)
        print("URL>>>>>>>>>>>",url)
        request.addValue(WebService.radar_auth_key.rawValue, forHTTPHeaderField: "Authorization")
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            if let data = data {
                do {
                    ...
                    print("json",json!)
                    let address = json?["address"] as? EIDictonary
                    if let ip = json?["ip"] as? String {
                        UserDefaults.standard.set(ip, forKey: "ip_address")
                    }
                    if let country_name = address?["country"] as? String{
    ...
    func fetchingLatitudeAndLongitudeValue(){
...
extension BaseViewController: CLLocationManagerDelegate {
    public func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
        ...
        case .authorizedAlways, .authorizedWhenInUse:
            fetchingLatitudeAndLongitudeValue()
        @unknown default:
            fatalError()
        }
    }
